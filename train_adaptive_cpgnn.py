#!/usr/bin/env python 
# -*- coding: utf-8 -*-
# @Describe: Training script for Adaptive Dynamic CP-GNN model
...
from models.adaptive_cp_gnn import AdaptiveDynamicCPGNN, PathLengthRegularization
from models.dynamic_cp_gnn import DynamicLinkPrediction, DynamicNodeClassification


def parse_args():
    parser = argparse.ArgumentParser(description='Train Adaptive Dynamic CP-GNN model')
    ...
    # Adaptive path parameters
    parser.add_argument('--max_path_length', type=int, default=4, help='Maximum context path length to consider')
    parser.add_argument('--use_structural', action='store_true', help='Use structural properties for path length prediction')
    parser.add_argument('--structural_dim', type=int, default=16, help='Dimension of structural properties')
    parser.add_argument('--entropy_weight', type=float, default=0.1, help='Weight for entropy regularization')
    parser.add_argument('--diversity_weight', type=float, default=0.1, help='Weight for diversity regularization')
    parser.add_argument('--path_reg_weight', type=float, default=0.01, help='Weight for path length regularization')
    ...
    parser.add_argument('--save_path', type=str, default='checkpoints/adaptive', help='Path to save models')
    parser.add_argument('--log_interval', type=int, default=10, help='Interval for logging')
    parser.add_argument('--visualize_attention', action='store_true', help='Visualize path attention weights')
...
def train_epoch(model, task_model, dataloader, optimizer, device, task, path_reg_weight):
    """Train for one epoch"""
    model.train()
    task_model.train()
    ...
    total_loss = 0
    total_task_loss = 0
    total_path_reg_loss = 0
    total_samples = 0
    
    task_criterion = nn.BCELoss() if task == 'link_prediction' else nn.CrossEntropyLoss()
    ...
    for batch in tqdm(dataloader, desc='Training'):
        ...
        # Forward pass through Adaptive Dynamic CP-GNN
        node_embeddings, path_attentions, path_reg_loss = model(input_adj, input_feats)
        ...
        if task == 'link_prediction':
            ...
            task_loss = task_criterion(predictions, labels)
            ...
            task_loss = task_criterion(logits.squeeze(0), node_labels)
        
        # Combined loss with path length regularization
        loss = task_loss + path_reg_weight * path_reg_loss
        ...
        total_path_reg_loss += path_reg_loss.item() * batch_size
        total_samples += batch_size
    
    avg_loss = total_loss / total_samples
    avg_task_loss = total_task_loss / total_samples
    avg_path_reg_loss = total_path_reg_loss / total_samples
    
    return avg_loss, avg_task_loss, avg_path_reg_loss


def evaluate(model, task_model, dataloader, device, task, path_reg_weight):
    """Evaluate model"""
    model.eval()
    task_model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='Evaluating'):
            ...
            # Store path attention weights for visualization
            all_path_attentions.append(path_attentions.cpu().numpy())
    ...
    all_path_attentions = np.concatenate(all_path_attentions, axis=0)
    
    metrics = {}
    metrics['loss'] = total_loss / total_samples
    metrics['task_loss'] = total_task_loss / total_samples
    metrics['path_reg_loss'] = total_path_reg_loss / total_samples
    ...
    return metrics, all_path_attentions


def visualize_path_attention(path_attentions, save_path):
    """Visualize path attention weights"""
    # Calculate average attention weights across batch and nodes
    avg_attention = np.mean(path_attentions, axis=(0, 2))  # Average over batch and nodes
    
    # Create figure for path attention
    plt.figure(figsize=(10, 6))
    
    # Plot average attention weights for each time step
    for t in range(avg_attention.shape[0]):
        plt.plot(range(1, avg_attention.shape[1] + 1), avg_attention[t], 
                label=f'Time step {t+1}', marker='o')
    
    plt.xlabel('Context Path Length')
    plt.ylabel('Average Attention Weight')
    plt.title('Average Attention Weight per Context Path Length')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(save_path, 'path_attention.png'))
    
    # Also visualize the distribution of preferred path lengths
    plt.figure(figsize=(10, 6))
    
    # For each node, find the path length with highest attention
    preferred_lengths = np.argmax(path_attentions, axis=-1) + 1  # +1 to make it 1-indexed
    
    # Flatten across batch and time dimensions
    preferred_lengths = preferred_lengths.reshape(-1)
    
    # Plot histogram
    plt.hist(preferred_lengths, bins=np.arange(0.5, path_attentions.shape[-1] + 1.5), alpha=0.7)
    plt.xlabel('Preferred Context Path Length')
    plt.ylabel('Count')
    plt.title('Distribution of Preferred Context Path Length')
    plt.xticks(np.arange(1, path_attentions.shape[-1] + 1))
    plt.grid(True)
    plt.savefig(os.path.join(save_path, 'preferred_path_length.png'))
...
def main():
    ...
    # Custom path length regularizer
    path_regularizer = PathLengthRegularization(
        entropy_weight=args.entropy_weight,
        diversity_weight=args.diversity_weight
    )
    
    # Create model
    model = AdaptiveDynamicCPGNN(
    ...
    for epoch in range(args.epochs):
        # Train for one epoch
        train_loss, train_task_loss, train_reg_loss = train_epoch(
            model, task_model, train_loader, optimizer, device, args.task, args.path_reg_weight
        )
        train_losses.append(train_loss)
        task_losses.append(train_task_loss)
        reg_losses.append(train_reg_loss)
        
        # Evaluate
        val_metrics_dict, path_attentions = evaluate(
            model, task_model, val_loader, device, args.task, args.path_reg_weight
        )
        ...
        if is_better:
            ...
            best_epoch = epoch
            best_path_attentions = path_attentions
        ...
        if (epoch + 1) % args.log_interval == 0:
            ...
            print(f"Epoch {epoch+1}/{args.epochs} | "
                  f"Train Loss: {train_loss:.4f} (Task: {train_task_loss:.4f}, Reg: {train_reg_loss:.4f}) | "
    ...
    # Visualize path attention if requested
    if args.visualize_attention:
        visualize_path_attention(best_path_attentions, args.save_path)
    ...
    # Final evaluation
    final_metrics, _ = evaluate(model, task_model, val_loader, device, args.task, args.path_reg_weight)
