...
class ACMDataLoader(GraphDataLoader):
    ...
    def load_raw_matrix(self):
        ...
        p_vs_p = data['PvsP']  # paper-paper (có thể là ma trận vuông)
        p_vs_a = data['PvsA']  # paper-author
        a_vs_f = data['AvsF']  # author-facility

    # Trích xuất chỉ số (source, destination) từ các ma trận không vuông
        src_p_a, dst_a = p_vs_a.nonzero()
        src_a_p, dst_p_a = p_vs_a.transpose().nonzero()

        src_p_l, dst_l = p_vs_l.nonzero()
        src_l_p, dst_p_l = p_vs_l.transpose().nonzero()

        src_a_f, dst_f = a_vs_f.nonzero()
        src_f_a, dst_a_f = a_vs_f.transpose().nonzero()

    # Với ma trận paper-paper (p_vs_p), giả sử là vuông -> dùng trực tiếp
        src_p_p, dst_p_p = p_vs_p.nonzero()

    # Tạo đồ thị dị thể
        graph_data = {
            ('p', 'pa', 'a'): (src_p_a, dst_a),
            ('a', 'ap', 'p'): (src_a_p, dst_p_a),
            ('p', 'pp', 'p'): (src_p_p, dst_p_p),
            ('p', 'ps', 's'): (src_p_l, dst_l),
            ('s', 'sp', 'p'): (src_l_p, dst_p_l),
            ('a', 'af', 'f'): (src_a_f, dst_f),
            ('f', 'fa', 'a'): (src_f_a, dst_a_f),
    }
...
class DBLPDataLoader(GraphDataLoader):
    ...
    # Tạo dictionary các cạnh của đồ thị
        graph_data = {
            ('p', 'pa', 'a'): (src_p_a, dst_a),
            ('a', 'ap', 'p'): (src_a_p, dst_p_a),
...
class IMDBDataLoader(GraphDataLoader):
    ...
    def load_raw_matrix(self):
        ...
        m_vs_a = data['m_vs_a']  # movie-actor
        m_vs_d = data['m_vs_d']  # movie-director
        m_vs_k = data['m_vs_k']  # movie-keyword

    # Trích xuất chỉ số (source, destination) từ các ma trận không vuông
        src_m_a, dst_a = m_vs_a.nonzero()
        src_a_m, dst_m_a = m_vs_a.transpose().nonzero()

        src_m_d, dst_d = m_vs_d.nonzero()
        src_d_m, dst_m_d = m_vs_d.transpose().nonzero()

        src_m_k, dst_k = m_vs_k.nonzero()
        src_k_m, dst_m_k = m_vs_k.transpose().nonzero()

    # Tạo dictionary các cạnh của đồ thị
        graph_data = {
            ('m', 'ma', 'a'): (src_m_a, dst_a),
            ('a', 'am', 'm'): (src_a_m, dst_m_a),
            ('m', 'md', 'd'): (src_m_d, dst_d),
            ('d', 'dm', 'm'): (src_d_m, dst_m_d),
            ('m', 'mk', 'k'): (src_m_k, dst_k),
            ('k', 'km', 'm'): (src_k_m, dst_m_k),
    }
